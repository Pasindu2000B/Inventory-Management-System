pipeline {
    ...
    stages {
        stage('SCM checkout') {
            steps {
                retry(3) {
                    checkout scmGit(branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/Pasindu2000B/Inventory-Management-System.git']])
                }
            }
        }
        stage('Build Docker Image') {
            steps {
                script {
                    sh 'docker build -t backend-app-image .'
                }
            }
        }
        stage('Run Docker Container') {
            steps {
                script {
                    ...
                    sh 'docker run -d -p 3001:3001 backend-app-image'
                }
            }
        }
